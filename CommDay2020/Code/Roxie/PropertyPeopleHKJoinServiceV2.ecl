...
Searchlayout := RECORD
 ROOT.File_People.Layout;
 STRING100 CompAddress;
END;
SearchLayout MergeSearch($.IDX.PropPayIDX Le,$.IDX.PeoplePayID Ri) := TRANSFORM
 SELF.CompAddress := STD.STR.CleanSpaces(
                     TRIM(Le.House_Number) + ' ' +
                     TRIM(Le.House_Number_suffix) + ' ' +
                     TRIM(Le.predir) + ' ' +
                     TRIM(Le.street) + ' ' +
                     TRIM(Le.streettype) + ' ' +
                     TRIM(Le.postdir) + ' ' +
...
GetPerson := JOIN($.IDX.PropPayIDX(house_number_key = '' OR TRIM(house_number,ALL) = TRIM(house_number_key,ALL), //Right Index
                 ...
                 MergeSearch(LEFT,RIGHT));
CHOOSEN(GetPerson,200);																	 
